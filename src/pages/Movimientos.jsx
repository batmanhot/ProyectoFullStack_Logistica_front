      ...
      <div className="bg-[#161d28] border border-white/8 rounded-xl p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input className={SI + ' pl-8 w-full'} placeholder="Buscar producto, SKU, doc, motivo..."
              value={busqueda} onChange={e => setBusqueda(e.target.value)}/>
          </div>
          <select className={SEL} style={{ width: 155 }} value={filtTipo} onChange={e => setFiltTipo(e.target.value)}>
            <option value="">Todos los tipos</option>
            {['ENTRADA', 'SALIDA', 'AJUSTE', 'TRANSFERENCIA', 'DEVOLUCION'].map(t => <option key={t}>{t}</option>)}
          </select>
          <select className={SEL} style={{ width: 165 }} value={filtAlm} onChange={e => setFiltAlm(e.target.value)}>
        ...
        <div className="overflow-x-auto rounded-xl border border-white/8">
          <table className="w-full border-collapse text-[13px]">
            <thead><tr>
              <TH c="Fecha"/><TH c="Tipo"/><TH c="Documento"/>
              <TH c="Producto"/><TH c="Almacén"/>
              <TH c="Cantidad" r/><TH c="Costo Unit." r/><TH c="Total" r/>
              <TH c="Motivo"/>
            </tr></thead>
            <tbody>
              {isLoading && (
                <tr><td colSpan={9} className="text-center text-[#5f6f80] py-8 text-[12px]">Cargando movimientos...</td></tr>
              )}
              {!isLoading && filtered.length === 0 && (
                <tr><td colSpan={9}>
                  <EmptyState icon={Boxes} title="Sin movimientos" description="No hay movimientos con los filtros actuales."/>
                </td></tr>
              )}
              {filtered.map(m => {
                const p   = productos.find(x => x.id === m.productoId)
                ...
                return (
                  <tr key={m.id} className="border-b border-white/6 last:border-0 hover:bg-white/2">
                    ...
                    <td className={`px-3.5 py-2.5 font-mono text-[12px] text-right ${esEnt ? 'text-green-400' : 'text-red-400'}`}>
                    ...
                    <td className="px-3.5 py-2.5 font-mono text-[12px] text-right text-[#9ba8b6]">
                      {formatCurrency(Number(m.costoUnitario || 0), simboloMoneda)}
                    </td>
                    <td className="px-3.5 py-2.5 font-mono text-[12px] text-right font-semibold text-[#e8edf2]">
