...
export default function Reportes() {
  ...
  const abcConAcum = useMemo(() =>
    abc.reduce((acc, p) => {
      const acum = (acc.length ? acc[acc.length-1].valorAcum : 0) + p.valorStock
      acc.push({ ...p, valorAcum: acum, pctAcum: valorTotal > 0 ? (acum / valorTotal) * 100 : 0 })
      return acc
    }, [])
  , [abc, valorTotal])
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-[#161d28] border border-white/8 rounded-xl p-5">
              ...
              <DataTable
                ...
                rowKey={r => r.id}
                columns={[
                  { key: 'sku', header: 'SKU', render: r => <span className="font-mono text-[11px] text-[#00c896]">{r.sku}</span> },
                  { key: 'nombre', header: 'Producto', render: r => <span className="text-[12px] max-w-40 truncate font-medium text-[#e8edf2] block">{r.nombre}</span> },
                  { key: 'pmp', header: 'P. Costo', align: 'right', render: r => <span className="font-mono text-[12px] text-[#9ba8b6]">{formatCurrency(r.pmp, simboloMoneda)}</span> },
                  { key: 'precioVenta', header: 'P. Venta', align: 'right', render: r => (
                      r.precioVenta > 0
                        ? <span className="font-mono text-[12px] text-[#e8edf2]">{formatCurrency(r.precioVenta, simboloMoneda)}</span>
                        : <span className="font-mono text-[11px] text-[#5f6f80]">Sin precio</span>
                    ) },
                  { key: 'unidadesVend', header: 'Uds. Vend.', align: 'right', render: r => <span className="font-mono text-[12px] text-[#9ba8b6]">{r.unidadesVend}</span> },
                  { key: 'ingresos', header: 'Ingresos', align: 'right', render: r => <span className="font-mono text-[12px] text-[#3b82f6] font-semibold">{formatCurrency(r.ingresos, simboloMoneda)}</span> },
                  { key: 'margenBruto', header: 'Margen S/', align: 'right', render: r => {
                      const color = r.margenPct === null ? '#5f6f80' : r.margenPct >= 20 ? '#22c55e' : r.margenPct >= 0 ? '#f59e0b' : '#ef4444'
                      return <span className="font-mono text-[12px] font-semibold" style={{ color }}>{r.margenPct !== null ? formatCurrency(r.margenBruto, simboloMoneda) : '—'}</span>
                    } },
                  { key: 'margenPct', header: 'Margen %', align: 'right', render: r => {
                      if (r.margenPct === null) return <span className="text-[11px] text-[#5f6f80]">—</span>
                      const color = r.margenPct >= 20 ? '#22c55e' : r.margenPct >= 0 ? '#f59e0b' : '#ef4444'
                      return <span className="font-mono text-[12px] font-semibold" style={{ color }}>{r.margenPct.toFixed(1)}%</span>
                    } },
                  { key: 'abc', header: 'ABC', align: 'right', render: r => <Badge variant={r.abc==='A'?'success':r.abc==='B'?'info':'neutral'}>{r.abc}</Badge> },
                ]}
                footerRow={rentabilidad.length>0 ? [
                  <span className="text-[11px] font-semibold text-[#5f6f80] uppercase tracking-wide">TOTAL</span>,
                  null, null, null, null,
                  <span className="font-mono text-[13px] text-[#3b82f6]">{formatCurrency(kpisRent.totalIngresos, simboloMoneda)}</span>,
                  <span className="font-mono text-[13px]" style={{ color: kpisRent.totalMargen >= 0 ? '#22c55e' : '#ef4444' }}>{formatCurrency(kpisRent.totalMargen, simboloMoneda)}</span>,
                  <span className="font-mono text-[13px]" style={{ color: kpisRent.margenPct >= 20 ? '#22c55e' : kpisRent.margenPct >= 0 ? '#f59e0b' : '#ef4444' }}>{kpisRent.margenPct.toFixed(1)}%</span>,
                  null,
                ] : undefined}
              />
            </div>
          ...
          <div className="bg-[#161d28] border border-white/6 rounded-xl p-5">
            <div className="text-[11px] font-semibold text-[#5f6f80] uppercase tracking-[0.06em] mb-3">¿Cómo se calcula la rentabilidad?</div>
        ...
        <div className="bg-[#161d28] border border-white/8 rounded-xl p-5">
          ...
          <DataTable
            ...
            columns={[
              ...
              { key: 'nombre', header: 'Producto', render: p => <span className="font-medium text-[#e8edf2] max-w-40 truncate block">{p.nombre}</span> },
              { key: 'catNombre', header: 'Categoría', render: p => <span className="text-[#9ba8b6]">{p.catNombre}</span> },
              { key: 'stockActual', header: 'Stock', align: 'right', render: p => <span className="font-mono text-[12px]">{p.stockActual}</span> },
              { key: 'unidadMedida', header: 'U.M.', render: p => <span className="text-[#9ba8b6]">{p.unidadMedida}</span> },
              { key: 'pmp', header: 'Costo PMP', align: 'right', render: p => <span className="font-mono text-[12px] text-[#9ba8b6]">{formatCurrency(p.pmp, simboloMoneda)}</span> },
              { key: 'precioVenta', header: 'P. Venta', align: 'right', render: p => (
                  (p.precioVenta||0) > 0
                    ? <span className="font-mono text-[12px] text-[#e8edf2]">{formatCurrency(p.precioVenta, simboloMoneda)}</span>
                    : <span className="font-mono text-[11px] text-[#5f6f80]">—</span>
                ) },
              { key: 'margenNum', header: 'Margen %', align: 'right', render: p => {
                  const color = p.margenNum === null ? '#5f6f80' : p.margenNum >= 20 ? '#22c55e' : p.margenNum >= 0 ? '#f59e0b' : '#ef4444'
                  return <span className="font-mono text-[12px] font-semibold" style={{ color }}>{p.margenNum !== null ? `${p.margenNum.toFixed(1)}%` : '—'}</span>
                } },
              { key: 'valorStock', header: 'Valor Stock', align: 'right', render: p => <span className="font-mono text-[12px] font-semibold text-[#00c896]">{formatCurrency(p.valorStock, simboloMoneda)}</span> },
              { key: 'pctTotal', header: '% Total', align: 'right', render: p => <span className="font-mono text-[12px] text-[#9ba8b6]">{valorTotal > 0 ? ((p.valorStock / valorTotal) * 100).toFixed(1) : 0}%</span> },
              { key: 'abc', header: 'ABC', align: 'right', render: p => <Badge variant={p.abc==='A'?'success':p.abc==='B'?'info':'neutral'}>{p.abc}</Badge> },
            ]}
          />
        </div>
      )}
      ...
      {tab === 'movimientos' && (
        <div className="bg-[#161d28] border border-white/8 rounded-xl p-5">
          <div className="flex items-center justify-between mb-1">
            <div className="text-[11px] font-semibold text-[#5f6f80] uppercase tracking-[0.06em]">Entradas y Salidas por Mes</div>
            <div className="flex items-center gap-2">
              <Btn variant="secondary" size="sm"
                onClick={async()=>{ await exportarReportesMovimientosXLSX(movMes, simboloMoneda) }}
                style={{background:'#1e7b47',color:'#fff',borderColor:'#1e7b47'}}>
                <Download size={13}/>Excel
              </Btn>
              <Btn variant="secondary" size="sm"
                onClick={async()=>{ await exportarReportesMovimientosPDF(movMes, simboloMoneda, config?.empresa) }}
                style={{background:'#c0392b',color:'#fff',borderColor:'#c0392b'}}>
                <Download size={13}/>PDF
              </Btn>
            </div>
      ...
      {tab === 'abc' && (
        <div className="bg-[#161d28] border border-white/8 rounded-xl p-5 flex flex-col gap-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Btn variant="secondary" size="sm"
                onClick={async()=>{ await exportarReportesABCXLSX(abc, valorTotal, simboloMoneda) }}
                style={{background:'#1e7b47',color:'#fff',borderColor:'#1e7b47'}}>
                <Download size={13}/>Excel
              </Btn>
              <Btn variant="secondary" size="sm"
                onClick={async()=>{ await exportarReportesABCPDF(abc, valorTotal, simboloMoneda, config?.empresa) }}
                style={{background:'#c0392b',color:'#fff',borderColor:'#c0392b'}}>
                <Download size={13}/>PDF
              </Btn>
            </div>
          ...
          <DataTable
            ...
            rowKey={p => p.id}
            columns={[
              { key: 'abc', header: 'Clase', render: p => <Badge variant={p.abc==='A'?'success':p.abc==='B'?'info':'neutral'}>{p.abc}</Badge> },
              { key: 'sku', header: 'SKU', render: p => <span className="font-mono text-[12px] text-[#00c896]">{p.sku}</span> },
              { key: 'nombre', header: 'Producto', render: p => <span className="font-medium text-[#e8edf2]">{p.nombre}</span> },
              { key: 'stockActual', header: 'Stock', align: 'right', render: p => <span className="font-mono text-[12px]">{p.stockActual} {p.unidadMedida}</span> },
              { key: 'valorStock', header: 'Valor', align: 'right', render: p => <span className="font-mono text-[12px] text-[#00c896] font-semibold">{formatCurrency(p.valorStock, simboloMoneda)}</span> },
              { key: 'acum', header: '% Acum.', align: 'right', render: p => <span className="font-mono text-[12px] text-[#9ba8b6]">{p.pctAcum.toFixed(1)}%</span> },
            ]}
          />

          {/* ════════════════════════════════════════════════════════
              GUÍA DIDÁCTICA — definición y uso de la clasificación ABC
              ════════════════════════════════════════════════════════ */}

          {/* Panel A: ¿Qué es la clasificación ABC? */}
          <div className="bg-[#161d28] border border-white/6 rounded-xl p-5">
            <div className="text-[11px] font-semibold text-[#5f6f80] uppercase tracking-[0.06em] mb-3">¿Qué es la Clasificación ABC?</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {[
                { cls:'A', pct:'80%', titulo:'Clase A — Alto valor',   desc:'Son el 20% de los productos pero representan el 80% del valor total. Requieren control riguroso, conteos físicos frecuentes, stock de seguridad alto y negociación activa con proveedores.' },
                { cls:'B', pct:'15%', titulo:'Clase B — Valor medio',  desc:'Representan el 15% del valor. Requieren control moderado, revisiones periódicas y reorden por punto de pedido estándar. Son importantes pero no críticos.' },
                { cls:'C', pct:'5%',  titulo:'Clase C — Bajo valor',   desc:'Son muchos productos pero solo el 5% del valor. Control mínimo, pedidos en lotes grandes para reducir costos operativos. Menor frecuencia de conteo físico.' },
              ].map(({ cls, pct, titulo, desc }) => (
                <div key={cls} className="bg-[#1a2230] rounded-lg p-3.5 border-l-2 border-[#00c896]/30">
                  <div className="text-[12px] font-semibold text-[#e8edf2] mb-1">{titulo} ({pct})</div>
          ...
          <div className="bg-[#161d28] border border-white/6 rounded-xl p-5">
            <div className="text-[11px] font-semibold text-[#5f6f80] uppercase tracking-[0.06em] mb-3">¿Cómo usar este análisis en tu negocio?</div>
            <div className="text-[12px] text-[#9ba8b6] leading-relaxed mb-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {[
                ...
                <div key={titulo} className="bg-[#1a2230] rounded-lg p-3.5 border-l-2 border-[#00c896]/30">
                  <div className="text-[12px] font-semibold text-[#e8edf2] mb-1.5">{titulo}</div>
                  <div className="flex flex-col gap-1.5">
                    {items.map((item, i) => (
                      <div key={i} className="flex items-start gap-2">
                        <div className="w-1 h-1 rounded-full mt-1.5 shrink-0 bg-[#00c896]/50"/>
                        <span className="text-[11px] text-[#9ba8b6] leading-snug">{item}</span>
                      </div>
                    ))}
                  </div>
                </div>
