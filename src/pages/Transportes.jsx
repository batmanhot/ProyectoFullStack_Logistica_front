...
function TabSeguimiento() {
  ...
  // Rango de fechas — por defecto mostrar últimos 7 días + futuros
  const hoy        = fechaHoy()
  ...
  const rutasActivas = useMemo(() =>
    rutas.filter(r => r.estado === 'EN_RUTA')
  , [rutas])

  const rutasProgramadas = useMemo(() =>
    rutas.filter(r => r.estado === 'PROGRAMADA')
  , [rutas])

  // Historial: todas las rutas en el rango, sin importar estado
  const rutasHistorial = useMemo(() => {
    return rutas
      .filter(r => {
        const f = r.fechaSalida || ''
        const desdeOk = !filtDesde || f >= filtDesde
        const hastaOk = !filtHasta || f <= filtHasta
  ...
  const SI = 'px-3 py-2 bg-[#1e2835] border border-white/[0.08] rounded-lg text-[13px] text-[#e8edf2] outline-none focus:border-[#00c896] font-[inherit]'

  return (
    <div className="flex flex-col gap-5">

      {/* ── Rutas EN RUTA ahora mismo ──────────────────── */}
      {rutasActivas.length > 0 ? (
        <div className="bg-[#161d28] border border-[#00c896]/25 rounded-xl overflow-hidden">
          ...
          <div className="flex flex-col divide-y divide-white/[0.05]">
            {rutasActivas.map(ruta => {
              const tra         = transportistas.find(t => t.id === ruta.transportistaId)
              const entregadas  = ruta.paradas.filter(p => p.estado === 'ENTREGADO').length
              const fallidas    = ruta.paradas.filter(p => p.estado === 'FALLIDO').length
              const enCamino    = ruta.paradas.filter(p => p.estado === 'EN_CAMINO').length
                ...
                <div key={ruta.id} className="p-5">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <div className="text-[12px] text-[#9ba8b6]">
                        🚚 {tra?.nombre}
                        {tra?.placa && <span className="font-mono ml-1.5 text-[#5f6f80]">{tra.placa}</span>}
                        {tra?.vehiculo && <span className="text-[#5f6f80]"> · {tra.vehiculo}</span>}
                      </div>
                      <div className="text-[11px] text-[#5f6f80] mt-0.5">
                        Salida: <strong className="text-[#9ba8b6]">{ruta.horaSalida}</strong>
                  ...
                  <div className="mb-4">
                    <div className="w-full h-3 bg-[#0e1117] rounded-full overflow-hidden flex">
                      {entregadas > 0 && (
                        <div className="h-full bg-green-500 transition-all" style={{ width:`${(entregadas/total)*100}%` }}/>
                      )}
                      {enCamino > 0 && (
                        <div className="h-full bg-blue-500 transition-all" style={{ width:`${(enCamino/total)*100}%` }}/>
                      )}
                      {fallidas > 0 && (
                        <div className="h-full bg-red-500 transition-all" style={{ width:`${(fallidas/total)*100}%` }}/>
                      )}
                    </div>
                  ...
                  {/* Paradas — tabla compacta */}
                  <div className="rounded-xl overflow-hidden border border-white/[0.06]">
                    <table className="w-full border-collapse text-[12px]">
                      <thead>
                        <tr className="bg-[#1a2230]">
                          <th className="px-3 py-2 text-left text-[10px] font-semibold text-[#5f6f80] uppercase border-b border-white/[0.06] w-8">#</th>
                          <th className="px-3 py-2 text-left text-[10px] font-semibold text-[#5f6f80] uppercase border-b border-white/[0.06]">Cliente</th>
                          <th className="px-3 py-2 text-left text-[10px] font-semibold text-[#5f6f80] uppercase border-b border-white/[0.06] hidden md:table-cell">Dirección</th>
                          <th className="px-3 py-2 text-left text-[10px] font-semibold text-[#5f6f80] uppercase border-b border-white/[0.06] w-24">Hora</th>
                          <th className="px-3 py-2 text-left text-[10px] font-semibold text-[#5f6f80] uppercase border-b border-white/[0.06] w-28">Estado</th>
                        </tr>
                      </thead>
                      <tbody>
                        {ruta.paradas.map((p, idx) => {
                          const des    = despachos.find(d => d.id === p.despachoId)
                          const bgRow  = p.estado === 'EN_CAMINO' ? 'bg-blue-500/5'
                                       : p.estado === 'ENTREGADO' ? 'bg-green-500/5 opacity-70'
                                       : p.estado === 'FALLIDO'   ? 'bg-red-500/5'
                                       : ''
                          const dotCol = p.estado === 'EN_CAMINO' ? '#3b82f6'
                                       : p.estado === 'ENTREGADO' ? '#22c55e'
                                       : p.estado === 'FALLIDO'   ? '#ef4444'
                                       : '#374151'
                          return (
                            <tr key={p.despachoId} className={`border-b border-white/[0.04] last:border-0 ${bgRow}`}>
                              ...
                              <td className="px-3 py-2.5 text-[#5f6f80] truncate max-w-[180px] hidden md:table-cell">
                                {des?.direccionEntrega || '—'}
                              </td>
                              <td className="px-3 py-2.5 font-mono text-[11px] text-[#9ba8b6]">
                                {p.horaLlegada
                                  ? <span className="text-green-400">{p.horaLlegada}</span>
                                  : p.estado === 'EN_CAMINO'
                                    ? <span className="text-blue-400 animate-pulse">En camino...</span>
                                    : '—'
                                }
                              </td>
                              <td className="px-3 py-2.5">
                  ...
                  {ruta.observaciones && (
                    <p className="mt-3 text-[11px] text-[#9ba8b6] italic px-1">{ruta.observaciones}</p>
                  )}
                </div>
              )
            })}
        ...
        <div className="bg-[#161d28] border border-white/[0.08] rounded-xl px-5 py-8 flex items-center justify-center gap-3 text-[#5f6f80]">
          <Truck size={20} className="opacity-40"/>
          <span className="text-[13px]">No hay rutas en tránsito en este momento</span>
        </div>
      )}

      {/* ── Rutas Programadas próximas ─────────────────── */}
        ...
        <div className="bg-[#161d28] border border-white/[0.08] rounded-xl overflow-hidden">
          ...
          <div className="divide-y divide-white/[0.05]">
            {rutasProgramadas.slice(0,5).map(ruta => {
              const tra = transportistas.find(t => t.id === ruta.transportistaId)
              return (
                <div key={ruta.id} className="flex items-center gap-4 px-5 py-3 hover:bg-white/[0.02]">
                  <div className="w-8 h-8 rounded-lg bg-[#5f6f80]/15 flex items-center justify-center shrink-0">
                    <Clock size={14} className="text-[#5f6f80]"/>
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-[#5f6f80]">
                      {formatDate(ruta.fechaSalida)} a las {ruta.horaSalida} · {ruta.paradas.length} parada{ruta.paradas.length !== 1 ? 's' : ''}
                    </div>
                  </div>
          ...
          <div className="px-5 py-10 flex flex-col items-center gap-3 text-[#5f6f80]">
            ...
            <div className="text-center">
              ...
              <p className="text-[11px] mt-0.5">
                {filtDesde && `Desde ${formatDate(filtDesde)}`}
                {filtDesde && filtHasta && ' · '}
                {filtHasta && `Hasta ${formatDate(filtHasta)}`}
                {!filtDesde && !filtHasta && 'Ajusta el rango de fechas'}
              </p>
            </div>
          </div>
        ) : (
              ...
              <thead>
                <tr className="bg-[#1a2230]">
                  {['N° Ruta','Fecha Salida','Transportista','Paradas','Entregas','Retorno','Estado'].map(h => (
                    <th key={h} className="px-4 py-2.5 text-left text-[10px] font-semibold text-[#5f6f80] uppercase tracking-[0.05em] border-b border-white/[0.06] whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {rutasHistorial.map(ruta => {
                  ...
                  const total      = ruta.paradas.length
                  return (
                    <tr key={ruta.id} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02]">
                      <td className="px-4 py-3 font-mono text-[12px] font-semibold text-[#00c896]">{ruta.numero}</td>
                      <td className="px-4 py-3 text-[#9ba8b6]">{formatDate(ruta.fechaSalida)} <span className="text-[#5f6f80] text-[11px]">{ruta.horaSalida}</span></td>
                      ...
                      <td className="px-4 py-3 font-mono text-[11px] text-[#9ba8b6]">
                        {ruta.horaRetorno ? `${ruta.horaRetorno}` : <span className="text-[#374151]">—</span>}
                      </td>
                      <td className="px-4 py-3">
                        <Badge variant={meta?.color || 'neutral'}><Icon size={9}/> {meta?.label || ruta.estado}</Badge>
                      </td>
                    </tr>
                  )
                })}
      ...
      <div className="bg-[#161d28] border border-blue-500/20 rounded-xl p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-[12px] text-[#9ba8b6] leading-relaxed mb-3">
              Muestra en tiempo real qué rutas están activas, el progreso de cada parada y el historial completo de salidas. El operador actualiza el estado de cada entrega desde la pestaña <strong className="text-[#e8edf2]">Rutas y Salidas → Gestionar</strong>.
            </p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {[
                ['🟢 Barra segmentada', 'Verde = entregado · Azul = en camino · Rojo = no entregado · Gris = pendiente'],
                ['📋 Historial', 'Filtra por rango de fechas. Por defecto muestra los últimos 7 días. Incluye todas las rutas sin importar su estado.'],
                ['🔄 ¿Cómo actualizar?', 'El conductor reporta por teléfono → operador abre la ruta en "Rutas y Salidas" → marca cada parada.'],
                ['📅 Sin datos', 'Si el historial está vacío, amplía el rango de fechas hacia atrás usando el campo "Desde".'],
              ].map(([t, d]) => (
                <div key={t} className="bg-[#1a2230] rounded-lg p-3">
                  <div className="text-[11px] font-semibold text-[#e8edf2] mb-1">{t}</div>
                  <div className="text-[11px] text-[#9ba8b6] leading-snug">{d}</div>
...
function MapaRuta({ paradas, despachos, clientes, almacenDireccion, horaSalida }) {
  ...
  // Se llama con delay externo para respetar el rate limit
  async function geocodificar(dir) {
    if (!dir?.trim()) return null
    try {
      const q = encodeURIComponent(dir + ', Lima, Peru')
      const r = await fetch(
    ...
    <Modal open title={`Ruta ${ruta.numero}`} onClose={onClose} size="lg"
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-[#00c896]/10 border border-[#00c896]/25 rounded-xl mb-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[13px] font-semibold text-[#e8edf2] truncate">
            {almacen?.nombre || config?.empresa || 'Almacén'}
          </div>
          <div className="text-[11px] text-[#9ba8b6] truncate">{almacenDir}</div>
        </div>
        <div className="text-right shrink-0">
          <div className="text-[10px] text-[#5f6f80] uppercase tracking-wide mb-0.5">Hora salida</div>
          <div className="text-[15px] font-bold text-[#e8edf2] font-mono">{ruta.horaSalida || '—'}</div>
