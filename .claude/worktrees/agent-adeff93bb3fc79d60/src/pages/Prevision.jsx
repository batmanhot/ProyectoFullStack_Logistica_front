  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col gap-5">
          ...
          <div className="flex items-center gap-2">
            ...
            {[7, 15, 30, 60].map(d => (
              <button key={d} onClick={() => setHorizonte(d)}
                className={`px-3 py-1.5 rounded-lg text-[12px] font-medium border transition-all ${horizonte === d ? 'bg-[#00c896]/10 border-[#00c896]/40 text-[#00c896]' : 'border-white/8 text-[#5f6f80] hover:text-[#9ba8b6]'}`}>
                {d}d
              </button>
            ))}
          </div>
      ...
      {prod && analisis?.sinDatos && (
        <div className="bg-[#161d28] border border-white/8 rounded-xl p-8 text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
            {[
              ...
              <div key={l} className="relative bg-[#161d28] border border-white/8 rounded-xl px-5 py-4 overflow-hidden">
                ...
                <div className={`text-[17px] font-semibold font-mono ${analisis.diasStock !== null && l.includes('Días') ? diasColor(analisis.diasStock) : 'text-[#e8edf2]'}`}>{v}</div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-[#161d28] border border-white/8 rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                ['Costo estimado de reposición', formatCurrency(analisis.eoq * (prod.precioCompra || 0), simboloMoneda), `${analisis.eoq} × PMP ${formatCurrency(prod.precioCompra || 0, simboloMoneda)}`],
